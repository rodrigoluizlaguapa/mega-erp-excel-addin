...
Office.onReady(() => {
    // Inicialização silenciosa da Ribbon
});

// --- FUNÇÕES DA RIBBON ---

async function gerarTemplateAgentes(event) {
  try {
    ...
    // ESSENCIAL PARA A RIBBON NÃO TRAVAR
    event.completed();
  }
}

async function processarInclusoesAgentes(event) {
  try {
    await Excel.run(async (context) => {
      const sheet = context.workbook.worksheets.getActiveWorksheet();
...
// Vinculando as funções EXCLUSIVAMENTE para a Ribbon
